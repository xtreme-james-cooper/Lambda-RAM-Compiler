...
fun restructure :: "unstr_state \<Rightarrow> flat_state" where
  "restructure (US h hp x p e ep vs vp sh sp 0) = 
    FS (H h hp) (H e ep) (listify' vs vp) []"
| "restructure (US h hp x p e ep vs vp sh sp (Suc pc)) = 
...
lemma [dest]: "FS h env vs sfs = restructure \<Sigma> \<Longrightarrow> \<exists>h' hp x p e ep vs' vp sh sp pc. 
  \<Sigma> = US h' hp x p e ep vs' vp sh sp pc \<and> h = H h' hp \<and> env = H e ep \<and> vs = listify' vs' vp \<and> 
...
proof (induction "restructure \<Sigma>\<^sub>u" \<Sigma>\<^sub>f' rule: evalf.induct)
  ...
  then obtain h' hp xx pp e ep vs' vp sh sp where S: "
    \<Sigma>\<^sub>u = US h' hp xx pp e ep vs' vp sh sp (Suc pc) \<and> h = H h' hp \<and> env = H e ep \<and> 
  ...
  have "iter (\<tturnstile> cd \<leadsto>\<^sub>u) (US h' hp xx pp e ep vs' vp sh sp (Suc pc)) \<Sigma>\<^sub>u' \<and> FS h env (v # vs) (pc # p # sfs) = restructure \<Sigma>\<^sub>u'" by simp
