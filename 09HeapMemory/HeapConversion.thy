...
abbreviation unheap_stack :: "closure\<^sub>h heap \<Rightarrow> (ptr list list \<times> nat) list \<Rightarrow> 
  ...
  "unheap_stack h s \<equiv> map (\<lambda>(\<Delta>, p). (map (map (unheap_closure h)) \<Delta>, p)) s"
...
proof (induction h x rule: unheap_closure.induct)
  ...
  proof (cases "hlookup h x")
    ...
    with 1 show ?thesis
    proof (cases "list_all (list_all ((>) x)) \<Delta>")
      case True
      from Lam\<^sub>h 1 have "list_all (list_all (hcontains h)) \<Delta>" 
        by (metis hlookup_all bounded_closure.simps(2))
      hence S: "\<And>vs y. vs \<in> set \<Delta> \<Longrightarrow> y \<in> set vs \<Longrightarrow> hcontains h y" by auto
      from 1 Lam\<^sub>h True have "\<And>y vs. vs \<in> set \<Delta> \<Longrightarrow> y \<in> set vs \<Longrightarrow> hcontains h y \<Longrightarrow> 
...
lemma halloc_unheap_list_env [simp]: "halloc h c = (h', v) \<Longrightarrow> 
  list_all (list_all (hcontains h)) \<Delta> \<Longrightarrow> heap_all (bounded_closure h) h \<Longrightarrow> 
    map (map (unheap_closure h')) \<Delta> = map (map (unheap_closure h)) \<Delta>"
  by (induction \<Delta>) (simp_all del: unheap_closure.simps)

lemma halloc_unheap_stack [simp]: "halloc h c = (h', v) \<Longrightarrow> 
  list_all (list_all (list_all (hcontains h))) (map fst s) \<Longrightarrow> heap_all (bounded_closure h) h \<Longrightarrow> 
...
lemma unheap_to_cons [simp]: "S\<^sub>b \<V>\<^sub>b ((\<Delta>, p) # s) = unheap \<Sigma>\<^sub>h \<Longrightarrow> \<exists>h \<V>\<^sub>h \<Delta>\<^sub>h s\<^sub>h. 
  \<Sigma>\<^sub>h = S\<^sub>h h \<V>\<^sub>h ((\<Delta>\<^sub>h, p) # s\<^sub>h) \<and> \<V>\<^sub>b = map (unheap_closure h) \<V>\<^sub>h \<and> 
    \<Delta> = map (map (unheap_closure h)) \<Delta>\<^sub>h \<and> s = unheap_stack h s\<^sub>h"
proof (induction \<Sigma>\<^sub>h)
  case (S\<^sub>h h \<V>\<^sub>h s\<^sub>h)
...
proof (induction "unheap \<Sigma>\<^sub>h" \<Sigma>\<^sub>b' rule: eval\<^sub>b.induct)
  case (ev\<^sub>b_lookup \<C> p x y \<Delta>\<^sub>b vs v \<V>\<^sub>b s\<^sub>b)
  then obtain h \<V>\<^sub>h \<Delta>\<^sub>h s\<^sub>h where "\<Sigma>\<^sub>h = S\<^sub>h h \<V>\<^sub>h ((\<Delta>\<^sub>h, Suc p) # s\<^sub>h) \<and> \<V>\<^sub>b = map (unheap_closure h) \<V>\<^sub>h \<and> 
    \<Delta>\<^sub>b = map (map (unheap_closure h)) \<Delta>\<^sub>h \<and> s\<^sub>b = unheap_stack h s\<^sub>h" by fastforce
  moreover with ev\<^sub>b_lookup obtain vs' v' where "lookup \<Delta>\<^sub>h x = Some vs' \<and> lookup vs' y = Some v' \<and> 
    v = unheap_closure h v'" by fastforce
  moreover with ev\<^sub>b_lookup have "\<C> \<tturnstile> S\<^sub>h h \<V>\<^sub>h ((\<Delta>\<^sub>h, Suc p) # s\<^sub>h) \<leadsto>\<^sub>h S\<^sub>h h (v' # \<V>\<^sub>h) ((\<Delta>\<^sub>h, p) # s\<^sub>h)" 
    by simp
