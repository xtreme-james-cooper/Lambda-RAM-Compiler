...
inductive eval\<^sub>h :: "code\<^sub>b list \<Rightarrow> state\<^sub>h \<Rightarrow> state\<^sub>h \<Rightarrow> bool" (infix "\<tturnstile> _ \<leadsto>\<^sub>h" 50) where
  ev\<^sub>h_lookup [simp]: "lookup \<C> p = Some (Lookup\<^sub>b x y) \<Longrightarrow> lookup \<Delta> x = Some vs \<Longrightarrow> 
...
| ev\<^sub>h_pushenv [simp]: "lookup \<C> p = Some (PushEnv\<^sub>b n) \<Longrightarrow> 
...
proof (induction \<C> \<Sigma> \<Sigma>' rule: eval\<^sub>h.induct)
  ...
  from ev\<^sub>h_lookup(4, 1, 2, 3) show ?case by (induction rule: eval\<^sub>h.cases) simp_all 
next
