...
definition unmap_mem :: "(register \<Rightarrow> nat \<Rightarrow> register \<times> nat) \<Rightarrow> nat \<Rightarrow> nat" where
  "unmap_mem m x = (case unmap_mem' x of (r, p) \<Rightarrow> pseudoreg_map (m r p))"

primrec inv_register_map :: "(register \<Rightarrow> nat) \<Rightarrow> register \<Rightarrow> reg \<Rightarrow> nat" where
...
next
  ...
  hence "disassemble cd \<tturnstile> MS (inv_register_map ps r) (unmap_mem mem) (Suc pc) \<leadsto>\<^sub>m 
    MS ((inv_register_map ps r)(R5 := unmap_mem mem (inv_register_map ps r R5))) 
      (unmap_mem mem) pc" by (metis evm_lod)
  with eva_geta(2, 3) show ?case by simp
next
  ...
  with eva_get(2, 3) show ?case by simp
next
...
lemma [simp]: "inv_register_map (case_register 0 0 0 2 0) Acc = case_reg 0 1 2 11 0" 
  by (auto split: reg.splits)

lemma [simp]: "inv_register_map (case_register hp ep (Suc 0) 0 0) Acc = 
...
lemma [simp]: "unmap_mem (case_register (\<lambda>x. (Acc, 0)) (\<lambda>x. (Acc, 0)) (\<lambda>x. (Acc, 0)) 
...
proof (rule, unfold unmap_mem_def)
  ...
  with R show "(case unmap_mem' x of (r, p) \<Rightarrow> 
    pseudoreg_map (case_register (\<lambda>x. (Acc, 0)) (\<lambda>x. (Acc, 0)) (\<lambda>x. (Acc, 0))
