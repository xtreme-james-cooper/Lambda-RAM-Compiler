...
fun typecheck :: "'a expr\<^sub>s \<rightharpoonup> ty expr\<^sub>s \<times> ty" where
  "typecheck e = (
    ...
    in case unify con of 
        Some sub \<Rightarrow> Some (map_expr\<^sub>s to_type (map_expr\<^sub>s (subst sub) e'), to_type (subst sub t))
...
lemma [simp]: "typecheck' \<Gamma> vs e = (e', t', vs', con) \<Longrightarrow> finite vs \<Longrightarrow> tyvars\<^sub>s e' \<subseteq> vs' - vs"
...
proof -
  assume "typecheck' \<Gamma> vs e = (e', t', vs', con)"
     and "finite vs"
  hence "tyvars\<^sub>s e' \<subseteq> vs' - vs" by simp
...
next
  ...
  from App\<^sub>s T1 have T1': "typecheck' (\<Gamma>(x \<mapsto> t')) (insert ?v vs) e1 = 
    (map_expr\<^sub>s (subst [y \<mapsto> t']) e\<^sub>1', subst [y \<mapsto> t'] t\<^sub>1, vs'', constr_subst y t' con\<^sub>1)" by blast
  from App\<^sub>s T2 A B C D have T2': "typecheck' (\<Gamma>(x \<mapsto> t')) vs'' e2 = 
    (map_expr\<^sub>s (subst [y \<mapsto> t']) e\<^sub>2', subst [y \<mapsto> t'] t\<^sub>2, vs''', constr_subst y t' con\<^sub>2)" by blast
  from App\<^sub>s E T1' T2' have "typecheck' (\<Gamma>(x \<mapsto> t')) vs (App\<^sub>s e1 e2) = 
    (map_expr\<^sub>s (subst [y \<mapsto> t']) e', subst [y \<mapsto> t'] tt, vs', constr_subst y t' con)" by (auto simp add: Let_def)
  thus ?case by blast
qed auto
...
next
  ...
  with tc\<^sub>t_app TC1 TC2 Y obtain s\<^sub>2 where S2: "map_expr\<^sub>s (eliminate_vars vs'') (map_expr\<^sub>s (subst s\<^sub>2) e\<^sub>2') = map_expr\<^sub>s to_unifiable e\<^sub>2 \<and> 
  ...
  from tc\<^sub>t_app TC1 TC2 V1 VG have VE2: "?v \<notin> tyvars\<^sub>s e\<^sub>2'" by simp
  from tc\<^sub>t_app TC1 have "tyvars\<^sub>s e\<^sub>1' \<subseteq> vs'' - insert ?v vs" by simp
  with D2 have HV: "dom s\<^sub>2 \<inter> tyvars\<^sub>s e\<^sub>1' = {}" by auto
  from tc\<^sub>t_app E TC1 TC2 have HV2: "tyvars\<^sub>s e\<^sub>2' \<subseteq> vs' - vs''" by simp
  ...
  with D2 S2 HV2 have HVS2: "tyvars\<^sub>s (map_expr\<^sub>s (subst s\<^sub>2) e\<^sub>2') = {}" by auto
