...
primrec declosure :: "closure\<^sub>c \<Rightarrow> expr\<^sub>d" where
  "declosure (Const\<^sub>c k) = Const\<^sub>d k"
| "declosure (Lam\<^sub>c t cs e) = multisubst (map declosure cs) (Lam\<^sub>d t e)"

fun declosure_stack :: "frame\<^sub>c list \<Rightarrow> frame\<^sub>k list" where
  "declosure_stack [] = []"
| "declosure_stack (FApp1\<^sub>c cs e # s) = FApp1\<^sub>k (multisubst (map declosure cs) e) # declosure_stack s"
| "declosure_stack (FApp2\<^sub>c c # s) = FApp2\<^sub>k (declosure c) # declosure_stack s"
| "declosure_stack (FReturn\<^sub>c cs # s) = FReturn\<^sub>k # declosure_stack s"

primrec destate\<^sub>c :: "state\<^sub>c \<Rightarrow> state\<^sub>k" where
  "destate\<^sub>c (SE\<^sub>c s cs e) = S\<^sub>k False (declosure_stack s) (multisubst (map declosure cs) e)"
| "destate\<^sub>c (SC\<^sub>c s c) = S\<^sub>k True (declosure_stack s) (declosure c)"
...
proof (induction c t and cs ts rule: typing_closure\<^sub>c_typing_environment\<^sub>c.inducts)
  case (tc\<^sub>c_lam cs ts t\<^sub>1 e t\<^sub>2)
...
theorem typesafec [simp]: "\<Sigma> :\<^sub>c t \<Longrightarrow> destate\<^sub>c \<Sigma> :\<^sub>k t"
proof (induction \<Sigma> t rule: typecheck_state\<^sub>c.induct)
...
theorem correctc: "\<Sigma> \<leadsto>\<^sub>c \<Sigma>' \<Longrightarrow> \<Sigma> :\<^sub>c t \<Longrightarrow> iter (\<leadsto>\<^sub>k) (destate\<^sub>c \<Sigma>) (destate\<^sub>c \<Sigma>')"
proof (induction \<Sigma> \<Sigma>' rule: eval\<^sub>c.induct)
  ...
  moreover from ev\<^sub>c_var have "lookup (map declosure cs) x = Some (declosure c)" by simp
  ultimately have "multisubst (map declosure cs) (Var\<^sub>d x) = declosure c" by (metis multisubst_var)
  thus ?case by simp
next
  case (ev\<^sub>c_lam s cs t e)
...
lemma [simp]: "FApp1\<^sub>k e # s = declosure_stack s\<^sub>c \<Longrightarrow> \<exists>cs e' s\<^sub>c'. s\<^sub>c = FApp1\<^sub>c cs e' # s\<^sub>c' \<and> 
...
lemma [simp]: "FApp2\<^sub>k e # s = declosure_stack s\<^sub>c \<Longrightarrow> 
    \<exists>c s\<^sub>c'. s\<^sub>c = FApp2\<^sub>c c # s\<^sub>c' \<and> e = declosure c \<and> s = declosure_stack s\<^sub>c'"
  by (induction s\<^sub>c rule: declosure_stack.induct) simp_all

lemma [simp]: "FReturn\<^sub>k # s = declosure_stack s\<^sub>c \<Longrightarrow> \<exists>cs s\<^sub>c'. s\<^sub>c = FReturn\<^sub>c cs # s\<^sub>c'"
  by (induction s\<^sub>c rule: declosure_stack.induct) simp_all

lemma [dest]: "S\<^sub>k b s e = destate\<^sub>c \<Sigma> \<Longrightarrow> 
  \<exists>s'. s = declosure_stack s' \<and> 
    ((\<exists>cs e'. \<not> b \<and> \<Sigma> = SE\<^sub>c s' cs e' \<and> e = multisubst (map declosure cs) e') \<or> 
      (\<exists>c. b \<and> \<Sigma> = SC\<^sub>c s' c \<and> e = declosure c))"
...
lemma lam_to_declosure [simp]: "Lam\<^sub>d t\<^sub>1 e = declosure c \<Longrightarrow> c :\<^sub>c\<^sub>l Arrow t' t\<^sub>2 \<Longrightarrow>
  \<exists>cs e'. c = Lam\<^sub>c t\<^sub>1 cs e' \<and> multisubst (map declosure cs) (Lam\<^sub>d t\<^sub>1 e') = Lam\<^sub>d t\<^sub>1 e \<and> 
    ([t\<^sub>1] \<turnstile>\<^sub>d e : t\<^sub>2) \<and> t' = t\<^sub>1 \<and>
    (\<forall>e\<^sub>2. ([] \<turnstile>\<^sub>d e\<^sub>2 : t\<^sub>1) \<longrightarrow> multisubst (map declosure cs) (subst\<^sub>d 0 e\<^sub>2 e') = subst\<^sub>d 0 e\<^sub>2 e)"
proof (induction c)
  case (Lam\<^sub>c tt cs e')
...
lemma [dest]: "Const\<^sub>d k = multisubst es (declosure c) \<Longrightarrow> c = Const\<^sub>c k"
proof (induction c)
  case (Lam\<^sub>c t cs e)
  moreover obtain e' where "multisubst (map declosure cs) (Lam\<^sub>d t e) = Lam\<^sub>d t e'" by fastforce
...
lemma [dest]: "Const\<^sub>d k = multisubst (map declosure cs) (Var\<^sub>d x) \<Longrightarrow> lookup cs x = Some (Const\<^sub>c k)"
  by (induction cs x rule: lookup.induct) auto

lemma [dest]: "Const\<^sub>d k = multisubst (map declosure cs) e \<Longrightarrow>
    e = Const\<^sub>d k \<or> (\<exists>x. e = Var\<^sub>d x \<and> lookup cs x = Some (Const\<^sub>c k))"
proof (induction e)
  case (Lam\<^sub>d t e)
...
proof (induction c t and cs ts arbitrary: and e tt 
  ...
  thus ?case by simp
next
  case (tc\<^sub>c_cons c t cs ts)
  hence "[] \<turnstile>\<^sub>d declosure c : t" by simp
  hence "ts \<turnstile>\<^sub>d declosure c : t" using tc_postpend by fastforce
  moreover from tc\<^sub>c_cons have "insert_at 0 t ts \<turnstile>\<^sub>d e : tt" by (cases ts) simp_all
...
proof (induction c t and cs ts arbitrary: and e rule: typing_closure\<^sub>c_typing_environment\<^sub>c.inducts)
  case tc\<^sub>c_nil
...
lemma [dest]: "cs :\<^sub>c\<^sub>l\<^sub>s ts \<Longrightarrow> lookup ts x = Some tt \<Longrightarrow> 
  Lam\<^sub>d t e = multisubst (map declosure cs) (Var\<^sub>d x) \<Longrightarrow> 
    \<exists>cs' e'. lookup cs x = Some (Lam\<^sub>c t cs' e') \<and> 
...
lemma [dest]: "cs :\<^sub>c\<^sub>l\<^sub>s ts \<Longrightarrow> ts \<turnstile>\<^sub>d e : t' \<Longrightarrow> Lam\<^sub>d t e' = multisubst (map declosure cs) e \<Longrightarrow> 
  (\<exists>e''. e = Lam\<^sub>d t e'') \<or> (\<exists>x cs' e''. e = Var\<^sub>d x \<and> lookup cs x = Some (Lam\<^sub>c t cs' e'') \<and> 
...
lemma [simp]: "value\<^sub>d (multisubst (map declosure cs) e) \<Longrightarrow> SE\<^sub>c s cs e :\<^sub>c t \<Longrightarrow>
  \<exists>c. SE\<^sub>c s cs e \<leadsto>\<^sub>c SC\<^sub>c s c \<and> multisubst (map declosure cs) e = declosure c"
proof (induction e)
  case (Var\<^sub>d x)
  then obtain t' ts where "(s :\<^sub>c t' \<rightarrow> t) \<and> (cs :\<^sub>c\<^sub>l\<^sub>s ts) \<and> (lookup ts x = Some t')" by fastforce
  then obtain c where "lookup cs x = Some c \<and> c :\<^sub>c\<^sub>l t'" by fastforce
  hence "SE\<^sub>c s cs (Var\<^sub>d x) \<leadsto>\<^sub>c SC\<^sub>c s c \<and> multisubst (map declosure cs) (Var\<^sub>d x) = declosure c" 
    by fastforce
  thus ?case by blast
next
  case (Const\<^sub>d k)
  have "SE\<^sub>c s cs (Const\<^sub>d k) \<leadsto>\<^sub>c SC\<^sub>c s (Const\<^sub>c k) \<and> 
    multisubst (map declosure cs) (Const\<^sub>d k) = declosure (Const\<^sub>c k)" by simp
  thus ?case by blast
next
  case (Lam\<^sub>d tt e)
  hence "SE\<^sub>c s cs (Lam\<^sub>d tt e) \<leadsto>\<^sub>c SC\<^sub>c s (Lam\<^sub>c tt cs e) \<and> 
    multisubst (map declosure cs) (Lam\<^sub>d tt e) = declosure (Lam\<^sub>c tt cs e)" by simp
  thus ?case by blast
qed simp_all

theorem completec [simp]: "destate\<^sub>c \<Sigma>\<^sub>c \<leadsto>\<^sub>k \<Sigma>\<^sub>s' \<Longrightarrow> \<Sigma>\<^sub>c :\<^sub>c t \<Longrightarrow> 
    \<exists>\<Sigma>\<^sub>c'. iter (\<leadsto>\<^sub>c) \<Sigma>\<^sub>c \<Sigma>\<^sub>c' \<and> \<Sigma>\<^sub>s' = destate\<^sub>c \<Sigma>\<^sub>c'"
proof (induction "destate\<^sub>c \<Sigma>\<^sub>c" \<Sigma>\<^sub>s' rule: eval\<^sub>k.induct)
  case (ev\<^sub>k_const s k)
  then obtain s' cs e' where S: "s = declosure_stack s' \<and> \<Sigma>\<^sub>c = SE\<^sub>c s' cs e' \<and> 
    Const\<^sub>d k = multisubst (map declosure cs) e'" by fastforce
  thus ?case
  proof (cases "e' = Const\<^sub>d k")
    case True
    have "SE\<^sub>c s' cs (Const\<^sub>d k) \<leadsto>\<^sub>c SC\<^sub>c s' (Const\<^sub>c k)" by simp
    hence "iter (\<leadsto>\<^sub>c) (SE\<^sub>c s' cs (Const\<^sub>d k)) (SC\<^sub>c s' (Const\<^sub>c k))" by (metis iter_refl iter_step)
    with S True show ?thesis by fastforce
  next
    case False
    with S obtain x where E: "e' = Var\<^sub>d x \<and> lookup cs x = Some (Const\<^sub>c k)" by fastforce
    hence "SE\<^sub>c s' cs (Var\<^sub>d x) \<leadsto>\<^sub>c SC\<^sub>c s' (Const\<^sub>c k)" by simp
    hence X: "iter (\<leadsto>\<^sub>c) (SE\<^sub>c s' cs (Var\<^sub>d x)) (SC\<^sub>c s' (Const\<^sub>c k))" by simp
    from S have "S\<^sub>k True s (Const\<^sub>d k) = destate\<^sub>c (SC\<^sub>c s' (Const\<^sub>c k))" by simp
...
next
  ...
  proof (cases "\<exists>e''. e' = Lam\<^sub>d tt e''")
    ...
    have "SE\<^sub>c s' cs (Lam\<^sub>d tt e'') \<leadsto>\<^sub>c SC\<^sub>c s' (Lam\<^sub>c tt cs e'')" by simp
    hence "iter (\<leadsto>\<^sub>c) (SE\<^sub>c s' cs (Lam\<^sub>d tt e'')) (SC\<^sub>c s' (Lam\<^sub>c tt cs e''))" 
      by (metis iter_refl iter_step)
    with S E show ?thesis by fastforce
  next
    case False
    with S T obtain x cs' e'' where E: "e' = Var\<^sub>d x \<and> lookup cs x = Some (Lam\<^sub>c tt cs' e'') \<and> 
      multisubst (map declosure cs') (Lam\<^sub>d tt e'') = Lam\<^sub>d tt e" by blast
    hence "SE\<^sub>c s' cs (Var\<^sub>d x) \<leadsto>\<^sub>c SC\<^sub>c s' (Lam\<^sub>c tt cs' e'')" by simp
    hence X: "iter (\<leadsto>\<^sub>c) (SE\<^sub>c s' cs (Var\<^sub>d x)) (SC\<^sub>c s' (Lam\<^sub>c tt cs' e''))" by simp
    from S E have "multisubst (map declosure cs) e' = multisubst (map declosure cs') (Lam\<^sub>d tt e'')"
      by metis
    with S have "S\<^sub>k True s (Lam\<^sub>d tt e) = destate\<^sub>c (SC\<^sub>c s' (Lam\<^sub>c tt cs' e''))" by simp
    with S E X show ?thesis by blast
  qed
next
  ...
  have "SE\<^sub>c s' cs (App\<^sub>d e\<^sub>1' e\<^sub>2') \<leadsto>\<^sub>c SE\<^sub>c (FApp1\<^sub>c cs e\<^sub>2' # s') cs e\<^sub>1'" by simp
  hence "iter (\<leadsto>\<^sub>c) (SE\<^sub>c s' cs (App\<^sub>d e\<^sub>1' e\<^sub>2')) (SE\<^sub>c (FApp1\<^sub>c cs e\<^sub>2' # s') cs e\<^sub>1')" 
    by (metis iter_refl iter_step)
...
next
  ...
  then obtain s\<^sub>c c where S: "FApp1\<^sub>k e\<^sub>2 # s = declosure_stack s\<^sub>c \<and> \<Sigma>\<^sub>c = SC\<^sub>c s\<^sub>c c \<and> e\<^sub>1 = declosure c"
    by fastforce
  then obtain cs' e\<^sub>2' s\<^sub>c' where S': "s\<^sub>c = FApp1\<^sub>c cs' e\<^sub>2' # s\<^sub>c' \<and> 
    e\<^sub>2 = multisubst (map declosure cs') e\<^sub>2' \<and> s = declosure_stack s\<^sub>c'" by fastforce
  have "SC\<^sub>c (FApp1\<^sub>c cs' e\<^sub>2' # s\<^sub>c') c \<leadsto>\<^sub>c SE\<^sub>c (FApp2\<^sub>c c # s\<^sub>c') cs' e\<^sub>2'" by simp
  hence "iter (\<leadsto>\<^sub>c) (SC\<^sub>c (FApp1\<^sub>c cs' e\<^sub>2' # s\<^sub>c') c) (SE\<^sub>c (FApp2\<^sub>c c # s\<^sub>c') cs' e\<^sub>2')" 
    by (metis iter_refl iter_step)
...
next
  ...
  with S' obtain cs' e\<^sub>1' where C: "c = Lam\<^sub>c t\<^sub>1 cs' e\<^sub>1' \<and> 
  ...
  with S S' have X: "S\<^sub>k False (FReturn\<^sub>k # s) (subst\<^sub>d 0 e\<^sub>2 e\<^sub>1) = 
    destate\<^sub>c (SE\<^sub>c (FReturn\<^sub>c (c\<^sub>2 # cs') # s\<^sub>c') (c\<^sub>2 # cs') e\<^sub>1')" by simp
  have "SC\<^sub>c (FApp2\<^sub>c (Lam\<^sub>c t\<^sub>1 cs' e\<^sub>1') # s\<^sub>c') c\<^sub>2 \<leadsto>\<^sub>c SE\<^sub>c (FReturn\<^sub>c (c\<^sub>2 # cs') # s\<^sub>c') (c\<^sub>2 # cs') e\<^sub>1'"
    by simp
...
next
  ...
  then obtain s\<^sub>c c where S: "FReturn\<^sub>k # s = declosure_stack s\<^sub>c \<and> \<Sigma>\<^sub>c = SC\<^sub>c s\<^sub>c c \<and> e = declosure c" 
    by fastforce
  then obtain cs s\<^sub>c' where S': "s\<^sub>c = FReturn\<^sub>c cs # s\<^sub>c'" by fastforce
  have "SC\<^sub>c (FReturn\<^sub>c cs # s\<^sub>c') c \<leadsto>\<^sub>c SC\<^sub>c s\<^sub>c' c" by simp
  hence "iter (\<leadsto>\<^sub>c) (SC\<^sub>c (FReturn\<^sub>c cs # s\<^sub>c') c) (SC\<^sub>c s\<^sub>c' c)" by (metis iter_refl iter_step)
  with S S' show ?case by fastforce
qed

lemma [simp]: "iter (\<leadsto>\<^sub>k) (destate\<^sub>c \<Sigma>\<^sub>c) \<Sigma>\<^sub>s' \<Longrightarrow> \<Sigma>\<^sub>c :\<^sub>c t \<Longrightarrow> 
  \<exists>\<Sigma>\<^sub>c'. iter (\<leadsto>\<^sub>c) \<Sigma>\<^sub>c \<Sigma>\<^sub>c' \<and> \<Sigma>\<^sub>s' = destate\<^sub>c \<Sigma>\<^sub>c'"
proof (induction "destate\<^sub>c \<Sigma>\<^sub>c" \<Sigma>\<^sub>s' arbitrary: \<Sigma>\<^sub>c rule: iter.induct)
  case (iter_step \<Sigma>\<^sub>s' \<Sigma>\<^sub>s'')
  moreover then obtain \<Sigma>\<^sub>c' where X: "iter (\<leadsto>\<^sub>c) \<Sigma>\<^sub>c \<Sigma>\<^sub>c' \<and> \<Sigma>\<^sub>s' = destate\<^sub>c \<Sigma>\<^sub>c'" by fastforce
  moreover with iter_step have "\<Sigma>\<^sub>c' :\<^sub>c t" by fastforce
  ultimately obtain \<Sigma>\<^sub>c'' where "iter (\<leadsto>\<^sub>c) \<Sigma>\<^sub>c' \<Sigma>\<^sub>c'' \<and> \<Sigma>\<^sub>s'' = destate\<^sub>c \<Sigma>\<^sub>c''" by fastforce
  with X have "iter (\<leadsto>\<^sub>c) \<Sigma>\<^sub>c \<Sigma>\<^sub>c'' \<and> \<Sigma>\<^sub>s'' = destate\<^sub>c \<Sigma>\<^sub>c''" by (metis iter_append)
  thus ?case by fastforce
qed force+

lemma [simp]: "value\<^sub>d (multisubst (map declosure cs) e) \<Longrightarrow> \<exists>c. SE\<^sub>c s cs e \<leadsto>\<^sub>c SC\<^sub>c s c" 
proof (induction e)
  case (Var\<^sub>d x)
  then obtain c where "lookup cs x = Some c" by (induction cs x rule: lookup.induct) simp_all
  hence "SE\<^sub>c s cs (Var\<^sub>d x) \<leadsto>\<^sub>c SC\<^sub>c s c" by simp
  thus ?case by fastforce
next
  case (Const\<^sub>d k)
  have "SE\<^sub>c s cs (Const\<^sub>d k) \<leadsto>\<^sub>c SC\<^sub>c s (Const\<^sub>c k)" by simp
...
next
  case (Lam\<^sub>d t e)
  have "SE\<^sub>c s cs (Lam\<^sub>d t e) \<leadsto>\<^sub>c SC\<^sub>c s (Lam\<^sub>c t cs e)" by simp
  thus ?case by fastforce
qed simp_all

lemma [simp]: "\<Sigma>\<^sub>c :\<^sub>c t \<Longrightarrow> iter (\<leadsto>\<^sub>k) (destate\<^sub>c \<Sigma>\<^sub>c) (S\<^sub>k True [] v) \<Longrightarrow> 
  \<exists>c. iter (\<leadsto>\<^sub>c) \<Sigma>\<^sub>c (SC\<^sub>c [] c) \<and> declosure c = v"
proof -
  assume TC: "\<Sigma>\<^sub>c :\<^sub>c t" and "iter (\<leadsto>\<^sub>k) (destate\<^sub>c \<Sigma>\<^sub>c) (S\<^sub>k True [] v)"
  then obtain \<Sigma>\<^sub>c' where E: "iter (\<leadsto>\<^sub>c) \<Sigma>\<^sub>c \<Sigma>\<^sub>c' \<and> S\<^sub>k True [] v = destate\<^sub>c \<Sigma>\<^sub>c'" by fastforce
  ...
  ultimately have S: "(\<exists>cs e. \<Sigma>\<^sub>c' = SE\<^sub>c [] cs e \<and> v = multisubst (map declosure cs) e) \<or> 
    (\<exists>c. \<Sigma>\<^sub>c' = SC\<^sub>c [] c \<and> v = declosure c)" by simp
