...
| "vals_from_stack (FPop\<^sub>c c # s\<^sub>c) = vals_from_stack s\<^sub>c"
...
| "prepend_to_top_env c ((\<Delta>, \<C>) # s\<^sub>e) = (c # \<Delta>, \<C>) # s\<^sub>e"
...
| "stack_from_stack (FPop\<^sub>c c # s\<^sub>c) = 
    prepend_to_top_frame [PopEnv\<^sub>e] (prepend_to_top_env (encode_closure c) (stack_from_stack s\<^sub>c))"
...
lemma encode_state_to_return [dest]: "encode_state \<Sigma>\<^sub>c = S\<^sub>e \<V> ((\<Delta>\<^sub>e, Return\<^sub>e # \<C>) # s\<^sub>e) \<Longrightarrow> 
  \<exists>\<Delta>\<^sub>c s c. \<Sigma>\<^sub>c = SC\<^sub>c (FReturn\<^sub>c \<Delta>\<^sub>c # s) c \<and> s\<^sub>e = stack_from_stack s \<and>
    \<Delta>\<^sub>e = map encode_closure \<Delta>\<^sub>c \<and> \<V> = encode_closure c # vals_from_stack s \<and> \<C> = []"
  by (induction \<Sigma>\<^sub>c) auto

lemma encode_state_to_jump [dest]: "encode_state \<Sigma>\<^sub>c = S\<^sub>e \<V> ((\<Delta>\<^sub>e, Jump\<^sub>e # \<C>) # s\<^sub>e) \<Longrightarrow> False"
  by (induction \<Sigma>\<^sub>c) auto
...
next
  ...
  then obtain s\<^sub>c c c' where "\<Sigma>\<^sub>c = SC\<^sub>c (FApp2\<^sub>c c' # s\<^sub>c) c \<and> 
    stack_from_stack s\<^sub>c = ((\<Delta>\<^sub>e, \<C>) # s\<^sub>e) \<and> v = encode_closure c \<and> 
    Lam\<^sub>e \<Delta>\<^sub>e' \<C>' = encode_closure c' \<and> \<V> = vals_from_stack s\<^sub>c" 
      by auto
...
next
  case (ev\<^sub>e_jump v \<Delta>\<^sub>e' \<C>' \<V> \<Delta>\<^sub>e \<C> s\<^sub>e)
  hence "encode_state \<Sigma>\<^sub>c = S\<^sub>e (v # Lam\<^sub>e \<Delta>\<^sub>e' \<C>' # \<V>) ((\<Delta>\<^sub>e, Jump\<^sub>e # \<C>) # s\<^sub>e)" by simp
  thus ?case by auto
qed
