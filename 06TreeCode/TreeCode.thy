...
to pop frames off the stack, rather than simply doing so when we run out of code. This will help 
make it clear where blocks end, once we no longer have the luxury of tree-structured code in the 
next stage - but the second operation \<open>Jump\<^sub>e\<close>, which we do not use yet, will also come in useful for 
tail-call optimization soon.\<close>

datatype return\<^sub>e =
  Return\<^sub>e
  | Jump\<^sub>e
...
datatype code\<^sub>e = 
  Lookup\<^sub>e nat
  | PushCon\<^sub>e nat
  | PushLam\<^sub>e "code\<^sub>e list" return\<^sub>e
  | Apply\<^sub>e

text \<open>Our closure-values remain the same, with just a change from a function body to a function 
codeblock and return operation in closures-proper.\<close>

datatype closure\<^sub>e = 
  Const\<^sub>e nat
  | Lam\<^sub>e "closure\<^sub>e list" "code\<^sub>e list" return\<^sub>e
...
type_synonym frame\<^sub>e = "closure\<^sub>e list \<times> code\<^sub>e list \<times> return\<^sub>e"
...
the code in a callstack frame is exhausted, we follow the \<open>Return\<^sub>e\<close> operation and pop that frame off 
the callstack. (The final \<open>ev\<^sub>e_jump\<close> transition we will cover later.)\<close>
...
inductive eval\<^sub>e :: "state\<^sub>e \<Rightarrow> state\<^sub>e \<Rightarrow> bool" (infix "\<leadsto>\<^sub>e" 50) where
  ev\<^sub>e_lookup [simp]: "lookup \<Delta> x = Some v \<Longrightarrow> 
    S\<^sub>e \<V> ((\<Delta>, Lookup\<^sub>e x # \<C>, r) # s) \<leadsto>\<^sub>e S\<^sub>e (v # \<V>) ((\<Delta>, \<C>, r) # s)"
| ev\<^sub>e_pushcon [simp]: "S\<^sub>e \<V> ((\<Delta>, PushCon\<^sub>e n # \<C>, r) # s) \<leadsto>\<^sub>e S\<^sub>e (Const\<^sub>e n # \<V>) ((\<Delta>, \<C>, r) # s)"
| ev\<^sub>e_pushlam [simp]: "S\<^sub>e \<V> ((\<Delta>, PushLam\<^sub>e \<C>' r' # \<C>, r) # s) \<leadsto>\<^sub>e 
    S\<^sub>e (Lam\<^sub>e \<Delta> \<C>' r' # \<V>) ((\<Delta>, \<C>, r) # s)"
| ev\<^sub>e_apply [simp]: "S\<^sub>e (v # Lam\<^sub>e \<Delta>' \<C>' r' # \<V>) ((\<Delta>, Apply\<^sub>e # \<C>, r) # s) \<leadsto>\<^sub>e 
    S\<^sub>e \<V> ((v # \<Delta>', \<C>', r') # (\<Delta>, \<C>, r) # s)"
| ev\<^sub>e_return [simp]: "S\<^sub>e \<V> ((\<Delta>, [], Return\<^sub>e) # s) \<leadsto>\<^sub>e S\<^sub>e \<V> s"
| ev\<^sub>e_jump [simp]: "S\<^sub>e (v # Lam\<^sub>e \<Delta>' \<C>' r' # \<V>) ((\<Delta>, [], Jump\<^sub>e) # s) \<leadsto>\<^sub>e S\<^sub>e \<V> ((v # \<Delta>', \<C>', r') # s)"
