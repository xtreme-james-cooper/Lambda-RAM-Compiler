...
datatype stack_state = SS bool "frame list" dexpr
...
inductive typecheck_stack_state :: "stack_state \<Rightarrow> ty \<Rightarrow> bool" (infix ":\<^sub>s" 50) where
  tcs_state [simp]: "s : t' \<rightarrow> t \<Longrightarrow> [] \<turnstile>\<^sub>d e : t' \<Longrightarrow> (b \<Longrightarrow> vald e) \<Longrightarrow> SS b s e :\<^sub>s t"
...
inductive evals :: "stack_state \<Rightarrow> stack_state \<Rightarrow> bool" (infix "\<leadsto>\<^sub>s" 50) where
  evs_const [simp]: "SS False s (DConst k) \<leadsto>\<^sub>s SS True s (DConst k)"
| evs_lam [simp]: "SS False s (DLam t e) \<leadsto>\<^sub>s SS True s (DLam t e)"
| evs_app1 [simp]: "SS False s (DApp e\<^sub>1 e\<^sub>2) \<leadsto>\<^sub>s SS False (FApp1 e\<^sub>2 # s) e\<^sub>1"
| evs_app2 [simp]: "SS True (FApp1 e\<^sub>2 # s) e\<^sub>1 \<leadsto>\<^sub>s SS False (FApp2 e\<^sub>1 # s) e\<^sub>2"
| evs_app3 [simp]: "SS True (FApp2 (DLam t e\<^sub>1) # s) e\<^sub>2 \<leadsto>\<^sub>s SS False (FReturn # s) (substd 0 e\<^sub>2 e\<^sub>1)"
...
proof (induction "[] :: ty list" e t rule: typecheckd.induct)
  case (tcd_const k)
  have "SS False s (DConst k) \<leadsto>\<^sub>s SS True s (DConst k)" by simp
  thus ?case by fastforce
next
  case (tcd_lam t\<^sub>1 e t\<^sub>2)
  have "SS False s (DLam t\<^sub>1 e) \<leadsto>\<^sub>s SS True s (DLam t\<^sub>1 e)" by simp
  thus ?case by fastforce
next
  case (tcd_app e\<^sub>1 t\<^sub>1 t\<^sub>2 e\<^sub>2)
  have "SS False s (DApp e\<^sub>1 e\<^sub>2) \<leadsto>\<^sub>s SS False (FApp1 e\<^sub>2 # s) e\<^sub>1" by simp
  thus ?case by fastforce
qed simp_all

lemma [simp]: "s : t' \<rightarrow> t \<Longrightarrow> [] \<turnstile>\<^sub>d e : t' \<Longrightarrow> vald e \<Longrightarrow> s = [] \<or> (\<exists>\<Sigma>'. SS True s e \<leadsto>\<^sub>s \<Sigma>')"
...
next
  ...
  moreover with tcs_cons_app2 have "SS True (FApp2 (DLam t\<^sub>1 e\<^sub>1') # s) e \<leadsto>\<^sub>s 
    SS False (FReturn # s) (substd 0 e e\<^sub>1')" by simp
...
theorem progresss: "\<Sigma> :\<^sub>s t \<Longrightarrow> (\<exists>e. \<Sigma> = SS True [] e \<and> vald e) \<or> (\<exists>\<Sigma>'. \<Sigma> \<leadsto>\<^sub>s \<Sigma>')"
...
proof (induction \<Sigma> \<Sigma>' rule: evals.induct)
  case (evs_const s k)
  thus ?case by (induction "SS False s (DConst k)" \<Sigma>'' rule: evals.induct) simp_all
next
  case (evs_lam s t e)
  thus ?case by (induction "SS False s (DLam t e)" \<Sigma>'' rule: evals.induct) simp_all
next
  case (evs_app1 s e\<^sub>1 e\<^sub>2)
  thus ?case by (induction "SS False s (DApp e\<^sub>1 e\<^sub>2)" \<Sigma>'' rule: evals.induct) simp_all
next
  case (evs_app2 e\<^sub>2 s e\<^sub>1)
  thus ?case by (induction "SS True (FApp1 e\<^sub>2 # s) e\<^sub>1" \<Sigma>'' rule: evals.induct) simp_all
next
  case (evs_app3 t e\<^sub>1 s e\<^sub>2)
  thus ?case by (induction "SS True (FApp2 (DLam t e\<^sub>1) # s) e\<^sub>2" \<Sigma>'' rule: evals.induct) simp_all
next 
  case (evs_ret s e)
...
lemma [simp]: "vald e \<Longrightarrow> iter (\<leadsto>\<^sub>s) (SS b s e) (SS True s e)"
proof (induction e)
  case (DConst x)
  thus ?case 
  proof (induction b)
    case False
    have "SS False s (DConst x) \<leadsto>\<^sub>s SS True s (DConst x)" by simp
...
next
  ...
  proof (induction b)
    case False
    have "SS False s (DLam t e) \<leadsto>\<^sub>s SS True s (DLam t e)" by simp
    thus ?case by (metis iter_step iter_refl)
  qed simp_all
qed simp_all
